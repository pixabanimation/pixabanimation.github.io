#!/usr/bin/env node
/**
 * generate-blog-index-from-fs.mjs
 *
 * Reads all blog HTML files directly from the filesystem, extracts metadata
 * from meta tags and JSON-LD, and generates blog/index.html.
 * No database connection required — works alongside the DB version.
 *
 * Usage: node tools/generate-blog-index-from-fs.mjs
 */
...
const assetsCoverDir = join(rootDir, 'assets', 'images', 'blog');
...
function coverUrl(cover) {
  ...
  const pngPath = join(assetsCoverDir, `${cover}.png`);
  const svgPath = join(assetsCoverDir, `${cover}.svg`);
  if (existsSync(pngPath)) return `${COVER_BASE}/${cover}.png`;
  if (existsSync(svgPath)) return `${COVER_BASE}/${cover}.svg`;
  return LOGO_URL;
}
...
function parseArticleMeta(filePath) {
  ...
  meta.slug = ogUrlMatch ? ogUrlMatch[1].replace(/https:\/\/[^/]+\/blog\//, '').replace('.html', '') : '';

  const titleMatch = html.match(/<meta property="og:title" content="([^"]+)">/);
  meta.title = titleMatch ? titleMatch[1] : '';

  const descMatch = html.match(/<meta name="description" content="([^"]+)">/);
  meta.excerpt = descMatch ? descMatch[1] : '';

  const imgMatch = html.match(/<meta property="og:image" content="([^"]+)">/);
  meta.cover = '';
  if (imgMatch) {
    let c = imgMatch[1];
    if (c.startsWith(COVER_BASE)) {
      c = c.slice(COVER_BASE.length + 1);
      c = c.replace(/\.(png|svg)$/, '');
      meta.cover = c;
    }
  }
  ...
  meta.category = sectionMatch ? sectionMatch[1] : 'Technology';
  ...
  meta.readingTime = timeMatch ? parseInt(timeMatch[1]) : 5;
...
function buildHtml(posts) {
  const sortedAll = [...posts].sort((a, b) => new Date(b.date) - new Date(a.date));
  const featured = sortedAll.slice(0, 3);

  const cats = {};
  for (const p of posts) {
    const c = p.category || 'Technology';
    cats[c] = (cats[c] || 0) + 1;
  }
  const categories = Object.entries(cats).map(([category, count]) => ({ category, count }));
...
<head>
  ...
  <title>PixabAnimation Blog — Motion Graphics, AI & Design Insights (${posts.length}+ Articles)</title>
  <meta name="description" content="Expert tutorials, creative insights, and industry trends on motion graphics, AI in design, animation, typography, freelancing, and creative technology. ${posts.length}+ articles from industry professionals.">
  ...
  <meta property="og:description" content="Expert tutorials, creative insights, and industry trends on motion graphics, AI, design, animation, tech reviews, and creative technology. ${posts.length}+ articles.">
  ...
  <meta name="twitter:description" content="Expert tutorials, creative insights, industry trends, and tech reviews — motion graphics, AI, design, animation, and gadgets. ${posts.length}+ articles.">
  ...
  <script type="application/ld+json">
  {
    ...
    "itemListElement": [
${sortedAll.map((p, i) => `      { "@type": "ListItem", "position": ${i + 1}, "url": "${BASE_URL}/blog/${p.slug}.html" }`).join(',\n')}
    ]
  }
  </script>
  <script type="application/ld+json">
      ...
      <ul class="nav-links" id="navLinks">
        <li class="nav-brand-item">
          <a href="${BASE_URL}/" class="nav-brand">
            <img src="${LOGO_URL}" alt="PixabAnimation" class="brand-logo" width="36" height="32">
          </a>
        </li>
    ...
    <div class="pl-hero-inner">
      ...
      <div class="pl-hero-stats">
        <div class="pl-hero-stat">
          <div class="pl-hero-stat-num">${posts.length}+</div>
...
${categories.map(c => `<a href="index.html" class="pl-cat-chip" data-cat="${escAttr(c.category)}" onclick="return plFilterByCat('${escAttr(c.category)}', this)" style="--cat-color: ${CAT_COLORS[c.category] || '#06c'}">${esc(c.category)}</a>`).join('\n')}
        ...
        <div class="pl-section-header">
          ...
          <span class="pl-section-count" id="plArticleCount">${posts.length} articles</span>
        </div>
...
${sortedAll.map(p => {
  ...
  return `          <a href="${p.slug}.html" class="pl-card" data-category="${escAttr(p.category)}" data-title="${escAttr(p.title.toLowerCase())}" data-excerpt="${escAttr(p.excerpt.toLowerCase())}" data-tags="${escAttr((p.tags || []).join(' ').toLowerCase())}" data-author="${escAttr(p.author.toLowerCase())}" data-search="${escAttr(searchable)}">
        ...
        <div style="text-align:center;margin-bottom:40px">
          <span class="pl-section-count" style="font-size:.85rem">Showing <span id="plVisibleCount" aria-live="polite">${posts.length}</span> of ${posts.length} articles</span>
        </div>
...
${sortedAll.slice(0, 5).map(p => {
  return `          <a href="${p.slug}.html" class="pl-sb-post">
            ...
            <div class="pl-sb-post-info">
              ...
              <div class="pl-sb-post-date">${fmtDate(p.date)} · ${p.readingTime} min</div>
            </div>
          </a>`;
}).join('\n')}
        </div>
...
${tags.map(t => `            <a href="index.html" class="pl-sb-tag" onclick="return plSearchTag('${esc(t.tag)}')">${esc(t.tag)}</a>`).join('\n')}
          </div>
        </div>
        <div class="pl-sb-widget">
          ...
          <div class="pl-sb-author">
            ...
            <div>
              ...
              <div class="pl-sb-author-role">Content Creator · ${posts.length} articles</div>
            </div>
          </div>
          <div class="pl-sb-author">
    ...
    <div class="footer-content">
      <div class="footer-grid">
        <div class="footer-brand">
          <img src="${LOGO_URL}" alt="PixabAnimation Logo" class="footer-logo" width="28" height="24" loading="lazy">
  ...
  <script>
    ...
    function plSearchKeydown(e){
      ...
      if(e.key==='ArrowUp'){e.preventDefault();plSetSuggestionIndex(plSuggestionIndex-1);return}
      if(e.key==='Enter'){e.preventDefault();plOpenSuggestion(plSuggestionIndex);return}
      if(e.key==='Escape'){plCloseSuggestions();return}
    }
    function plShowSuggestions(){
      var inp=plGetInput();
      if(!inp) return;
      plRenderSuggestions(inp.value);
    }
    function plApplyFilters(opts){
      opts=opts||{};
      var q=plNorm(plGetInput()?plGetInput().value:'');
      var cards=Array.prototype.slice.call(document.querySelectorAll('#plArticlesGrid .pl-card'));
      var visible=0;
      cards.forEach(function(card){
        var match=true;
        if(plCurrentCat!=='all'&&plCurrentCat!=='All'&&card.getAttribute('data-category')!==plCurrentCat) match=false;
        if(q&&card.getAttribute('data-search').indexOf(q)===-1) match=false;
        card.style.display=match?'':'none';
        if(match) visible++
      });
      var countEl=document.getElementById('plVisibleCount');
      if(countEl) countEl.textContent=visible;
      var articleCount=document.getElementById('plArticleCount');
      if(articleCount) articleCount.textContent=visible+' of '+cards.length+' articles';
...
function main() {
  console.log('🔍 Scanning blog directory for articles...');
  const files = readdirSync(blogDir)
    .filter(f => f.endsWith('.html') && f !== 'index.html' && f !== 'blog.css')
    .sort();
  ...
  for (const file of files) {
    ...
    try {
      ...
      console.warn(`   ⚠️  Error parsing ${file}: ${err.message}`);
    }
  }

  if (posts.length === 0) {
    console.error('❌ No valid articles found. Cannot generate index.');
    process.exit(1);
  }

  console.log(`   Parsed ${posts.length} articles successfully`);

  const html = buildHtml(posts);
  writeFileSync(outputPath, html, 'utf-8');
  console.log(`   ✅ Written: blog/index.html (${(html.length / 1024).toFixed(1)} KB)`);
